...
const userLog = (...message) => {
    ...
    console.info(`[${time}]`.gray, '[UserLog]'.cyan, message.join(' '));
    fileContent += [`[${time}]`.gray, '[UserLog]'.cyan, message.join(' ')].join(' ') + '\n';
 ...
 * @param {string[]} message 
 */
const roleLog = (...message) => {
    const time = new Date().toLocaleTimeString();
    let fileContent = fs.readFileSync('./terminal.log', 'utf-8');

    console.info(`[${time}]`.gray, '[RoleLog]'.magenta, message.join(' '));
    fileContent += [`[${time}]`.gray, '[RoleLog]'.magenta, message.join(' ')].join(' ') + '\n';
